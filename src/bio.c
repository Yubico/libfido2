    ...
    const char *pin, const fido_blob_t *token, int *ms)
{
	...
	if (pin && !token) {
		if ((r = bio_get_uv_token(dev, pin, cmd, &token_store, ms)) != FIDO_OK)
			goto fail;
    ...
    const char *pin, int *ms)
{
	...
	if ((r = bio_tx(dev, CMD_ENUM, NULL, 0, pin, fido_dev_puat_blob(dev), ms))
	    != FIDO_OK ||
    ...
    fido_bio_enroll_t *e, uint32_t timo_ms, const char *pin)
{
	...
	puat = fido_dev_puat_blob(dev);

	if ((pin == NULL && puat == NULL) || !fido_blob_is_empty(&e->token))
		return (FIDO_ERR_INVALID_ARGUMENT);

	if (puat)
		r = fido_blob_set(&e->token, puat->ptr, puat->len);
	else
