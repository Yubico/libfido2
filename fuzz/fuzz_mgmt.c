...
dev_get_cbor_info(const struct param *p)
{
	...
	r = fido_cbor_info_decrypt(ci, (const unsigned char *)p->pin1, /* XXX */
	    strlen(p->pin1));
	consume_str(fido_strerr(r));
	...
	for (size_t i = 0; i < fido_cbor_info_reset_transports_len(ci); i++) {
		char * const *sa = fido_cbor_info_reset_transports_ptr(ci);
		consume(sa[i], strlen(sa[i]));
	}
