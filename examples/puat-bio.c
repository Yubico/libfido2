...
typedef enum list_bio_action list_bio_templates_cb_t(
	const fido_bio_template_t *, fido_dev_t *, void *opaq);
...
list_bio_templates(fido_dev_t *dev, list_bio_templates_cb_t *cb, void *opaq)
{
	...
	if ((templates = fido_bio_template_array_new()) == NULL) {
		warn("fido_bio_template_array_new");
		return -1;
	}
	...
	for (size_t idx = 0; idx < fido_bio_template_array_count(templates);
		...
		if (cb(templ, dev, opaq) == LIST_BIO_STOP)
			break;
	}
...
enroll_new(fido_dev_t *dev, fido_bio_template_t **templ_out)
{
	...
	if ((templ = fido_bio_template_new()) == NULL)
		err(EX_OSERR, "fido_bio_template_new");
	if ((enroll = fido_bio_enroll_new()) == NULL)
		err(EX_OSERR, "fido_bio_enroll_new");
...
search_template(const fido_bio_template_t *t, fido_dev_t *dev, void *opaq)
{
...
run(fido_dev_t *dev, const char *pin)
{
	...
	r = fido_dev_get_puat(dev, CTAP21_UV_TOKEN_PERM_BIO, NULL, pin);
	if (r) {
	...
	if (r)
		return r;
	if (ctx.found) {
		warnx("unexpectedly found template");
		r = -1;
		goto exit;
	}
