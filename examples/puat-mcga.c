...
prepare_cred(fido_cred_t **cred_out)
{
	...
	if ((cred = fido_cred_new()) == NULL)
		errx(EXIT_FAILURE, "fido_cred_new");
...
prepare_assert(fido_assert_t **assert_out)
{
	...
	if ((assert = fido_assert_new()) == NULL)
		err(EXIT_FAILURE, "fido_assert_new");
...
run(fido_dev_t *dev, const char *pin)
{
	...
	r = fido_dev_get_puat(dev, FIDO_PUAT_MAKECRED | FIDO_PUAT_LARGEBLOB,
	    RPID, pin);
	if (r) {
	...
	r = fido_dev_get_puat(dev, FIDO_PUAT_GETASSERT | FIDO_PUAT_LARGEBLOB,
	    RPID, pin);
	if (r) {
		warnx("could not get puat: %s", fido_strerr(r));
		return r;
