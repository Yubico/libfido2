 ...
 */
static /* const */ unsigned char zlib_blob[694] = {
...
/*
 * expected sha256 of zlib_blob after decompression
 */
const unsigned char zlib_hash[SHA256_DIGEST_LENGTH] = {
...
const size_t zlib_origsiz = 1322;
...
zlib(void)
{
	...
	in.ptr = zlib_blob;
	in.len = sizeof(zlib_blob);

	assert(fido_uncompress(&out, &in, zlib_origsiz) == FIDO_OK);
	assert(out.len == zlib_origsiz);
	assert(fido_sha256(&dgst, out.ptr, out.len) == 0);
	assert(dgst.len == sizeof(zlib_hash));
	assert(memcmp(zlib_hash, dgst.ptr, dgst.len) == 0);
}
